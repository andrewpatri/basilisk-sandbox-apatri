...
double* sol_start2; // for ceramic phase
...
for (int jj=0; jj<NGS; jj++) {
  ...
  sol_start2 = (double *)malloc(NSS * sizeof(double));
...
event init (i = 0) {
  if (!restarted) {
    ...
    for (int jj=0; jj<NSS; jj++)
      sum += sol_start2[jj];
    if (fabs(sum-1.) > 1e-6) {
      fprintf(stderr, "Sum of solid fractions2 is not 1. Exiting...\n");
      exit(1);
    }
    ...
    foreach() {
      for (int jj=0; jj<NSS; jj++) {
        scalar YS = YSList[jj];
        if (y < Y_inert){ // define where we have a certain composition
        YS[] = sol_start[jj]*f[];
        } else {
        YS[] = sol_start2[jj]*f[];  
        }
...
event cleanup (t = end) {
  ...
  free(sol_start2),  sol_start2 = NULL;
