...
int main() {
 ...
 G.y = -9.81;
 L0 = H0*20.001; // first try just the block of wood
...
origin (0, 0);
  ...
  shift_prod = true;
  kinfolder = "biomass/dummy-gas-2507";
...
event init(i=0) {
  ...
  fraction (f, superquadric(x, y, 20, H0, 4*H0));
...
event output (t += 1) {
  ...
  char name[80];
  sprintf(name, "OutputData_gravity-293");
 ...
 double rate = 0;
 rate = (solid_mass_old - solid_mass)*2.*1e3/AREA_FACCIA; // g/m2/s 
